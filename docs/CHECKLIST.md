...
- [x] Unmatched duel refunds exactly, verified on-chain — `pnpm e2e`, tx `0x7f9a38bd…`
...
| M3 | Deployed + e2e script | **done** — escrow `0xbfaaf7b0…` on Shannon; two wallets completed a duel, tx hashes recorded |
...
| M7 | Edge cases | unmatched refund and loser-redeems-zero both confirmed ON-CHAIN |

# Known open ends

1. **§9 unknowns are all ANSWERED** (`docs/UNKNOWNS.md`). The duel design holds.
